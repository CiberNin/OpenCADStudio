...
#[test]
fn contacts_and_intersection_work_are_bounded() {
    for (a, b, expected) in [
        ([[0.0, 0.0], [1.0, 1.0]], [[1.0, 1.0], [2.0, 0.0]], false),
        ([[0.0, 0.0], [2.0, 0.0]], [[1.0, 0.0], [1.0, 1.0]], true),
        ([[0.0, 0.0], [2.0, 0.0]], [[1.0, 0.0], [3.0, 0.0]], true),
    ] {
        assert_eq!(crossings(&[a, b], &|p| p, &mut 10).unwrap(), [expected; 2]);
    }
    let edges = vec![[[0.0, 0.0], [1.0, 1.0]]; 100];
    let mut budget = 7;
    assert!(crossings(&edges, &|p| p, &mut budget).is_none());
    assert_eq!(budget, 0);
}
...
fn recovery_respects_the_total_hatch_vertex_cap() {
    ...
    assert!(refine(&original, || vec![curve], project).unwrap().len() > original.len());
