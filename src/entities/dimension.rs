...
fn dimension_text_entity(
    ...
    // A limits stack draws each half at DIMTFAC × DIMTXT, like AutoCAD.
...
fn dimension_text_parts(
    ...
    // stack is the value itself: AutoCAD prints the upper limit over the
...
/// DIMTZIN drops trailing zeros only as far as both values allow. AutoCAD
...
/// as in AutoCAD, and is handled by `limits_text`.
...
mod limits_format_tests {
    ...
    // AutoCAD's DIMLIM shows the upper limit over the lower one and nothing
