...
pub(super) fn on_open_file(&mut self) -> Task<Message> {
        ...
        let result = (|| {
            ...
            for name in selected {
                ...
                let page = if dialog.area == "Layout" {
                    ...
                    area_page.ok_or_else(|| crate::tf!("Layout '{name}' plot area is empty.").into_owned())?
                };
                pages.push(page);
            }
            Ok(pages)
    ...
    fn on_plot_dlg_commit(&mut self, preview: bool) -> Task<Message> {
        ...
        if d.area != "Layout" {
            ...
            if preview {
                ...
                let work = move || {
                    crate::io::pdf_export::export_pdf(&page, &tmp).and_then(|_| crate::io::print_to_printer::open_in_viewer(&tmp))
            ...
            let work = move || {
                crate::io::pdf_export::export_pdf(&page, &tmp).and_then(|_| crate::io::print_to_printer::print_existing_pdf(&tmp, &opts))
        ...
        let work = move || {
            iced::futures::executor::block_on(
                    crate::io::print_to_printer::print_wires_with(page, opts))
                .map(|printer| crate::tf!("Sent to printer: {printer}").into_owned())
                .map_err(|error| crate::tf!("Print failed: {error}").into_owned())
        };
    ...
    fn area_plot_job(&self, window: (f64, f64, f64, f64)) -> Option<PdfPageInput> {
        ...
        content.group_splits.wires = content.wires[..content.group_splits.wires.min(content.wires.len())]
            .iter().filter(|wire| in_window(wire)).count();
