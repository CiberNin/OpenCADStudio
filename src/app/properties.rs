                ...
                1 => {
                    ...
                    if let acadrust::EntityType::Dimension(d) = entity {
                        if let Some(general) = sections.first_mut() {
                            ...
                            let statuses = self.tabs[i].scene.dimension_association_status(d.base().common.handle);
                            let slots = self.tabs[i].scene.dimension_association_slot_points(d.base().common.handle).len();
                            let status = if statuses.is_empty() {
                                t!("Nonassociative")
                            } else if statuses.iter().any(|(_, status)| matches!(status, crate::scene::ReferenceStatus::Broken(_))) {
                                t!("Broken reference")
                            } else if statuses.iter().any(|(_, status)| matches!(status, crate::scene::ReferenceStatus::Unresolved)) {
                            ...
                            general.props.push(crate::scene::model::object::Property {
                                ...
                                value: crate::scene::model::object::PropValue::ReadOnly(status.into_owned()),
                            });
                            general.props.push(crate::scene::model::object::Property {
                                label: t!("Associative").into_owned(),
                                field: "associative",
