...
/// `Scene::constraint_glyph_hit` (`src/scene/sketch_constraints.rs`) is the
/// only caller — it maps the index back to a `ConstraintId`, since this
/// plain-data layout math has no reason to know about that scene-layer type.
...
impl canvas::Program<Message> for SelectionCanvas {
    ...
    fn draw(
        ...
        // Click-to-select hit testing against these pills lives in
        // `Scene::constraint_glyph_hit`, sharing this same size formula so
        // the clickable area never drifts from what's drawn here.
