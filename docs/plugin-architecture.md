...
**`acadrust_source` is checked differently, and this matters:** unlike
`rustc_version`, the check only runs if the plugin's `plugin.toml` declares
`acadrust_source` at all — the same `if !acadrust_declared { return true; }`
(or equivalent inline match) appears in **two** places,
`ExternalPlugin::acadrust_compatible` in `src/plugin/external.rs` (used when
loading an installed plugin) and the inline check inside
`fetch_release_info` in `src/plugin/marketplace.rs` (used when listing
releases in the Plugin Manager) — a manifest that omits the key entirely is treated
as compatible, not rejected. In practice this means the ABI gate is
opt-in per plugin rather than enforced across the board: a `plugin.toml` copied
from an older template that never added the field sails through unchecked.
**Always declare `acadrust_source`** (see the worked example below) so the gate
actually applies to your plugin; don't rely on the host to catch its absence.
...
xdata_apps = []
# Both filled at release-build time, not by hand — see the CI step below.
# Declare both; omitting acadrust_source silently skips that half of the
# compatibility check instead of failing it (see "Build-time ABI metadata"
# above), which defeats the point of declaring either field.
...
**Known gap: typed/pasted commands still get uppercased even when a leading
`>` is used to keep spaces literal.** The interactive command line
(`Message::CommandInput` in `src/app/update/mod.rs`) unconditionally calls
`s.to_uppercase()` on typed input unless a free-form text prompt is active
(`is_free_text_active()` — cell text, `TEXT`/`MTEXT` bodies). The `>` prefix
and `literal_spaces` mode (added since this was first found) correctly stop
a typed space from auto-submitting the line early — solving multi-word
arguments — but neither one skips the uppercase call, so a plugin command
needing a case-sensitive argument (a file path with mixed case, an embedded
expression, anything not meant to be all-caps) still gets mangled when
typed or pasted directly, even with `>`. Automation (`--serve`/`--mcp`)
bypasses this input path entirely and is unaffected — this is specifically
a typed/pasted-into-the-command-line issue. `ModuleEvent::PluginFileDialog`
sidesteps it for file paths (dispatches back with original case intact, as
noted above) but there's no equivalent for a plugin command that needs
case-sensitive *typed* text more generally. Fix direction: extend whatever
condition currently gates `literal_spaces`/`>` to also skip the uppercase
call, rather than treating "keep spaces literal" and "keep case literal" as
solved by the same escape when a plugin command may need one without the
other.
...
**Known gap: one linked repo is treated as one plugin's release history, even
when it hosts several.** `fetch_releases`/`fetch_release_info`
(`src/plugin/marketplace.rs`) fetch and parse *every* release under a linked
`owner/repo` into one flat `Vec<ReleaseInfo>`, with no filter on the parsed
manifest's own `id` field — which is already available at that point, since
each release's `plugin.toml` is downloaded and parsed to build `ReleaseInfo`
in the first place. A repo that ships more than one plugin (tag-namespaced
releases like `pluginA-v1.0.0`, `pluginB-v1.0.0`) will surface every other
plugin's releases as if they belonged to whichever one the user is looking
at, and any future "is there a newer version" check that walks this same
list by date/tag would need the same fix. A user can still install correctly
by picking the right tagged release (each release's own bundled `plugin.toml`
is what actually gets installed), but the discovery/version list itself
doesn't distinguish plugins sharing a repo. Fix direction: filter
`fetch_release_info`'s result (or its caller) to releases whose manifest
`id` matches the plugin currently being viewed/installed, the same `id`
`ReleaseInfo` already carries.
...
host process itself — but process isolation alone does **not** mean a
toolchain mismatch is safe to ignore: the plugin and host still exchange data
across the shared `ocs_plugin_api` types over IPC, and a struct-layout or ABI
difference between toolchain versions could corrupt that exchange in ways
subtler than an immediate crash. That's why, independently of process
isolation, API v4+ *also* enforces an exact `rustc_version` match (and,
correctly declared, `acadrust_source` — see above) before loading a plugin at
all, rather than relying on isolation to make a mismatch merely "someone
else's problem."
...
- [ ] Close the `acadrust_source` enforcement gap noted above (see
      "Build-time ABI metadata") — an omitted field is currently accepted as
      compatible instead of rejected, unlike `rustc_version`.
